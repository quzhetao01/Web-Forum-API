...
const AddPost = (): JSX.Element => {
    ...
    useEffect(() => {
        if (!localStorage.getItem('jwt')) { 
            ...
            .then(res => {
                // For authentication, user object not required here
            })
            .catch(err => { 
                //redirect to welcome page if error
                console.log(err);
                window.location.href = "http://localhost:3001/";
            })
            // set categories array for users to choose to tag their thread
    ...
    function addThread() {
        ...
        // if all the fields are filled up properly
        if (text.title && text.description && chosenCategory) {
            ...
            .then(res => {
                ...
                window.location.href = `http://localhost:3001/forum/${res.data.id}`;
            })
            .catch(err => {
                console.log(err);
            })
        ...
        // reset error messages (For scenario where user fails multiple times)
        ...
        <div className={"container py-5"}>
            <div className={`${AddPostCSS.container} card`}>
                ...
                <div className="card-body">
                    ...
                    {/* Error Messages, hidden unless new thread is submitted wrongly */}
